...
class RAGRequest(BaseModel):
    ...
    prompt_retrieve: Optional[str] = Field(
        default=RETRIEVE_PROMPT, description="Промпт для retrieval"
    )
    prompt_augmentation: Optional[str] = Field(
        default=MAP_REDUCE_PROMPT, description="Промпт для расширения запроса"
    )
    prompt_generation: Optional[str] = Field(
        default=GENERATE_FINAL_ANSWER_PROMPT, description="Промпт для генерации ответа"
    )

    top_k: Optional[int] = Field(
        default=12, ge=1, description="Сколько документов возвращает retriever"
    )
    temperature: Optional[float] = Field(
        default=0.7, ge=0.0, le=1.0, description="Температура генерации LLM"
    )
    threshold: Optional[float] = Field(
        default=0.0, ge=0.0, le=1.0, description="Порог для фильтрации по similarity"
    )

    document_id: UUID = Field(
        ..., description="ID документов, к которым привязан чат/пользователь"
    )

    llm: Optional[str] = Field(
        default="gpt-4o-mini", description="Имя LLM, используемой для генерации"
    )
...
class RAGResponse(BaseModel):
    ...
    message_id: Optional[str] = Field(
        default=None, description="ID сообщения из запроса, если был указан"
    )
    document_id: str = Field(..., description="ID документа из запроса")

    generated_answer: Optional[str] = Field(
        default=None, description="Сгенерированный ответ системы"
    )
