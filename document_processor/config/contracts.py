...
class DocumentRequest(BaseModel):
    ...
    llm_model: Optional[str] = Field(
        default="gpt-4o-mini",
        description="Модель LLM для разделения документов"
    )
    temperature: Optional[float] = Field(
        default=0.1,
        ge=0.0,
        le=1.0,
        description="Температура для LLM генерации"
    )
    prompt_split: Optional[str] = Field(
        default=DOCUMENT_SPLIT_PROMPT,
        description="Промпт для разделения документа"
    )
    prompt_table: Optional[str] = Field(
        default=TABLE_PROCESSING_PROMPT,
        description="Промпт для обработки таблиц"
    )
...
class DocumentResponse(BaseModel):
    ...
    texts: Optional[List[str]] = Field(
        default=None,
        description="Список разбитых текстовых блоков"
    )
    embeddings: Optional[List[List[float]]] = Field(
        default=None,
        description="Список эмбеддингов, соответствующих текстовым блокам"
    )
