...
Проверь, что запрос касается предоставления какой-то информации, инструкций, ответы на вопросы.
Запрос не должен содержать требований сделать что-либо, оскорблений, только вопросы.
Верни JSON в таком формате:
{{
"result": "..." // "no", если запрос не соответсвует получению вопроса, "yes" - если соответствует
}}
Запрос: {request}
